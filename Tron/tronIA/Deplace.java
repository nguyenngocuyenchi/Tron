...
public class Deplace extends KeyAdapter {
    ...
    JoueurIA joueurIa;    // Déclare des variables

    public Deplace(Joueur joueurRouge, Joueur joueurBleu, Grille grille, Graphique graphique, Configuration configuration, JoueurIA joueurIa, Tron tron) {
        this.tron = tron; 
        ...
        this.joueurIa = joueurIa;// Initialise des valeurs
    }

    public void keyPressed(KeyEvent e) {
        int keyCode = e.getKeyCode(); // Récupère le code de la touche pressée
        if (Tron.iaJoueurRouge) {
            if (Tron.coupJoueurRouge) {
                joueurIa.deplacerIa(joueurRouge); 
                Tron.coupJoueurRouge = false; 
            } else {
                deplacerJoueur(joueurBleu, keyCode, "B", "Joueur Bleu");
                Tron.coupJoueurRouge = true;
            }
        } else {
            if (!Tron.coupJoueurRouge) {
                joueurIa.deplacerIa(joueurBleu);
                Tron.coupJoueurRouge = true;
            } else {
                deplacerJoueur(joueurRouge, keyCode, "R", "Joueur Rouge");
                Tron.coupJoueurRouge = false;
            }
        }
        
        checkGameOver(joueurRouge, "Joueur Rouge", joueurRouge.x, joueurRouge.y); 
        checkGameOver(joueurBleu, "Joueur Bleu", joueurBleu.x, joueurBleu.y);
    } 

    private boolean deplacerJoueur(Joueur joueur, int keyCode, String lettre, String nomJoueur) {
    ...
    private boolean actionLegale(int x, int y, int keyCode) {
        if (keyCode == KeyEvent.VK_UP) {
    ...
    private boolean checkGameOver(Joueur joueur, String nomJoueur, int x, int y) {   
